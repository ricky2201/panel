...
$stmt = sqlsrv_query($conn, "SELECT UserID, UserName, UserEmail, UserPoint, VotePoint, PlayTime, UserLoginState FROM dbo.UserInfo WHERE UserID = ?", [$userid]);
...
<body class="bg-gray-900 text-white min-h-screen flex flex-col" x-data="{ open: false }">
  ...
  <main class="flex-grow max-w-6xl mx-auto py-10 px-4 grid grid-cols-1 md:grid-cols-3 gap-8">
    ...
    <!-- Sisi kanan (dengan style width 100%) -->
    <div class="space-y-6">
     <div class="flex flex-col space-y-3">
        <a href="topup.php" class="block bg-green-600 text-white py-2 rounded font-bold hover:bg-green-700 text-center uppercase" style="width: 150%;">Topup</a>
        <a href="topup_history_user.php" class="block bg-yellow-600 text-white py-2 rounded font-bold hover:bg-yellow-700 text-center uppercase" style="width: 150%;">Topup History</a>
  ...
  <?php include_once __DIR__ . '/../components/loading.php'; ?>
